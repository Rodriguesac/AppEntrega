...
fun ProfileScreen(
    ...
    BasePage("Perfil", "Conta do entregador", onBack, bottomBar = { AppBottomBar(AppRoute.Perfil, onNav) }) {
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            AddressBlock("Nome", driver?.nome ?: "Entregador", if (driver?.verificado == true) "Verificado profissional" else "Aguardando validação")
            AddressBlock("Telefone", driver?.telefone ?: "Não informado", "Para alterar, envie solicitação ao gestor.")
            AddressBlock("Recebimento", driver?.pixTipo ?: "Pix não cadastrado", driver?.pixChave ?: "Sem chave Pix")
